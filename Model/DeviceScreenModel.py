...
class DeviceScreenModel:
    ...
    def __init__(self, screen_transition_service, loop, **kwargs) -> None:
        ...
        self._observers = []
        self.send_parameters = SendParametersService()
    ...
    async def task_volt(self):
        ...
        while True:
            try:
                ...
                voltage_obr = int(round((int(GetVoltService.battery_voltage) - 2458)/982 * 100, 0))
    ...
    async def send_anode_task(self, n):
        if str(n) != DeviceScreenModel.parameters[3]:
            DeviceScreenModel.parameters[3] = await self.send_parameters.send_anode(DeviceScreenModel.ble_client, n)
    ...
    async def send_amp_task(self, value):
        if str(value) != DeviceScreenModel.parameters[0]:
            ...
            await self.send_parameters.send_amp(DeviceScreenModel.ble_client, value)
    ...
    async def task_send_dlit(self, value):
        if str(value) != DeviceScreenModel.parameters[1]:
            ...
            await self.send_parameters.send_dlit(DeviceScreenModel.ble_client, value)
    ...
    async def task_send_chast(self, value):
        if str(value) != DeviceScreenModel.parameters[2]:
            ...
            await self.send_parameters.send_chast(DeviceScreenModel.ble_client, value)
    ...
    async def task_send_segment(self, n):
        if str(n) != DeviceScreenModel.parameters[4]:
            ...
            await self.send_parameters.send_segment(DeviceScreenModel.ble_client, n)
